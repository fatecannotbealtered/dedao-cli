...
```bash
# Clone
git clone https://github.com/fatecannotbealtered/dedao-cli.git
cd dedao-cli
...
Release standard: **Functional Contract Coverage = 100%**. Every public behavior documented in `README`, `SKILL`, reference pages, `dedao-cli reference`, `--help`, `context`, `doctor`, `changelog`, or `update` must have automated command-level tests.
...
Keep `dedao-cli reference` `release_readiness` and `dedao-cli doctor`'s `release_readiness` check honest when test evidence changes.
...
5. **SKILL** — add a `skills/dedao-cli/reference/<domain>.md` page and link it from `skills/dedao-cli/SKILL.md` (keep SKILL.md a short progressive-disclosure index).
6. **Docs** — update `README.md` / `README_zh.md` command lists and add a line under `## [Unreleased]` in `CHANGELOG.md`.

`reference` walks the command tree automatically, so new commands appear in `dedao-cli reference` without extra wiring.
